...
export function listProjects(): KanbanProjectMeta[] {
  const db = getDb();
  const rows = db
    .prepare(
...
export function upsertProject(project: KanbanProject): void {
  ...
  const upsert = db.transaction(() => {
    ...
    // Replace tasks (and sub-tables)
    // First delete dependent tables via task IDs
    ...
    const insertLink = db.prepare(
      "INSERT INTO task_links (task_id, label, url, type) VALUES (?, ?, ?, ?)"
    );
    ...
    for (const task of project.tasks) {
      ...
      for (const link of task.links || []) {
        insertLink.run(task.id, link.label, link.url, link.type);
      }
      for (const dep of task.dependencies || []) {
        insertDep.run(task.id, dep.taskId, dep.type);
      }
    }
    ...
    const insertSessionTask = db.prepare(
      "INSERT OR IGNORE INTO session_tasks (session_id, task_id) VALUES (?, ?)"
    );
    for (const session of project.sessions || []) {
      ...
      for (const tid of session.taskIds || []) {
        // Only insert if the task exists
        const taskExists = db
          .prepare("SELECT 1 FROM tasks WHERE id = ?")
          .get(tid);
        if (taskExists) {
          insertSessionTask.run(session.id, tid);
        }
      }
    }

    // Update diagram links
    db.prepare("UPDATE diagrams SET project_id = NULL WHERE project_id = ?").run(
      project.id
    );
    if (project.diagramIds?.length) {
      const linkDiag = db.prepare(
        "UPDATE diagrams SET project_id = ? WHERE id = ?"
      );
      for (const did of project.diagramIds) {
        linkDiag.run(project.id, did);
      }
    }
  });
